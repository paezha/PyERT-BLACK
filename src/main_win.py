"""
Module Name: PyERT-BLACK Main Module(Windows version)
Source Name: main_win.py
Creator: All PyERT-BLACK project team members
Requirements: Python 3.8 or later
Date Created: Feb 15, 2023
Last Revised: Mar 20, 2023
Description: Main module of PyERT-BLACK that contains the main function for Windows

Version History:
2023-03-06 (main_win.py) Create main_win.py by copying code from PyERT-BLACK Main Module and update
    the code for visualizing routes generated and activity locations' information to make main 
    function prints URLs for visualization in console as hyperlinks instead of directly opening 
    the URLs in browser

2023-03-17 (main_win.py) Move get_points_boundary, extract_networkdata_pbf, extract_networkdata_bbox 
...
2023-03-17 (main_win.py) Update code for adding activity locations' information due to changes made in
    Activity Locations Identification moule

2023-03-18 (main_win.py) Update code for visualizing routes generated and activity locations' information 
    to let the program print excepion message and keep progressing when visualization failed instead of 
    crashing

2023-03-19 (main_win.py) Update code for extracting network, potential activity locations and landuse data 
...
def gen_hyperlink(url, label=None):
    if label is None: 
        label = url
    parameters = ''
...
will be collected from(optional, default 100 meters): ")
    ...
    if len(trip_gdf) >= 2 and (network_g != None):
        ...
        try:
            ...
            print("Unable to generate url to visualize " 
                  + "generated routes due to exception: \n" + str(e))

        routes_url = viz_json
        #webbrowser.open(routes_url, new=0, autoraise=True)
        print('Route generation done, to visualize route generated, click ', gen_hyperlink(routes_url, 'HERE'))
    else:
    ...
    if len(aloc_gdf) >= 1:
        ...
        #    aloc_gdf.to_json())
        #print('See stop segment points, click ', gen_hyperlink(aloc_points_url, 'HERE'))
        ...
        try:
            ...
            print("Unable to generate url to visualize the " 
                  + "activity locations' info due to exception: \n" + str(e))

        # print(viz_aloc_json)
        aloc_url = viz_aloc_json
        #webbrowser.open(aloc_url, new=0, autoraise=True)
        print('Adding PAL and LU info done, to visualize polygon activity locations, click ', gen_hyperlink(aloc_url, 'HERE'))
    else:
