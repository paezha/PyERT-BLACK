...
def gen_hyperlink(uri, label=None):
    if label is None: 
        label = uri
    parameters = ''
...
def extract_networkdata_bbox(max_lat, min_lat, max_lon, min_lon, mode):
    try:
        if mode not in ['drive', 'walk', 'all', 'bike']:
            raise NetworkModeError
    except NetworkModeError:
        print(
...
def main():
    ...
    if len(aloc_gdf) >= 1:
        ...
        print('Adding PAL and LU info done, to visualize activity locations, click ', gen_hyperlink(aloc_url, 'HERE'))
    else:
